...
4. **レンダラー** — three.js 側は状態を構造的ビューとして読むだけで描画する。ボールのシーム回転・水帯スラブ・水面流れのスパース矢じりも描画のみ
...
- **水中水帯**：テーブル下段 104px が水面線を持つ水深。水滴落下の衝撃が決定論的な流れ場（拡散＋減衰）を攪拌し、球には相対抗力／絶対減衰で作用、浮力で水面へ戻す。RNG は落水瞬間のみ消費するためシードの決定論を壊さない
